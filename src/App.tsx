...
import { SalaryProgression } from './components/SalaryProgression';
...
const App: React.FC = () => {
  ...
  // Selected employee for Global Profile Modal
  ...
  useEffect(() => {
      const loadData = async () => {
          ...
          } catch (error) { loadFromLocalStorage(); } finally { setIsLoaded(true); }
      };
      const loadFromLocalStorage = () => {
          const savedUsers = localStorage.getItem('users'); if (savedUsers) setUsers(JSON.parse(savedUsers));
          const savedEmps = localStorage.getItem('employees'); if (savedEmps) setEmployees(JSON.parse(savedEmps));
          const savedLeaves = localStorage.getItem('leaves'); if (savedLeaves) setLeaves(JSON.parse(savedLeaves));
          const savedLoans = localStorage.getItem('loans'); if (savedLoans) setLoans(JSON.parse(savedLoans));
          const savedReviews = localStorage.getItem('reviews'); if (savedReviews) setReviews(JSON.parse(savedReviews));
          const savedDepts = localStorage.getItem('departments'); if (savedDepts) setDepartments(JSON.parse(savedDepts));
          const savedAttendance = localStorage.getItem('attendance'); if (savedAttendance) setAttendance(JSON.parse(savedAttendance));
          const savedPenalties = localStorage.getItem('penalties'); if (savedPenalties) setPenalties(JSON.parse(savedPenalties));
          const savedBonuses = localStorage.getItem('bonuses'); if (savedBonuses) setBonuses(JSON.parse(savedBonuses));
          const savedTaxDebts = localStorage.getItem('taxDebts'); if (savedTaxDebts) setTaxDebts(JSON.parse(savedTaxDebts));
          const savedExternal = localStorage.getItem('externalWorkers'); if (savedExternal) setExternalWorkers(JSON.parse(savedExternal));
          setSaveStatus('local');
      };
      loadData();
  }, []);
  ...
  // --- SAVE DATA ---
  useEffect(() => {
      if (!isLoaded || isInitialMount.current) { isInitialMount.current = false; return; }
      ...
      const saveData = async () => {
          ...
          try {
              const fullData = { users, employees, leaves, loans, reviews, departments, attendance, penalties, bonuses, taxDebts, externalWorkers };
              const response = await fetch('http://localhost:3001/api/save', {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify(fullData)
              });
              if (response.ok) setSaveStatus('saved'); else setSaveStatus('error');
          } catch (error) { setSaveStatus('error'); }
      };
      const timeoutId = setTimeout(saveData, 1000);
      return () => clearTimeout(timeoutId);
  }, [users, employees, leaves, loans, reviews, departments, attendance, penalties, bonuses, taxDebts, externalWorkers, isLoaded]);
  ...
  const handleLogout = () => { sessionStorage.removeItem('currentUser'); setCurrentUser(null); setActivePage(Page.DASHBOARD); };
  const updateUserPassword = (newPassword: string) => {
      ...
      setUsers(updatedUsers); setCurrentUser({ ...currentUser, password: newPassword });
  };

  const renderContent = () => {
    switch (activePage) {
      case Page.DASHBOARD:
        return <Dashboard employees={employees} attendance={attendance} leaves={leaves} loans={loans} reviews={reviews} bonuses={bonuses} penalties={penalties} taxDebts={taxDebts} setPage={setActivePage} />;
      case Page.EMPLOYEES:
        return <EmployeeList employees={employees} setEmployees={setEmployees} departments={departments} onEmployeeClick={(emp) => setSelectedProfileEmployee(emp)} />;
      case Page.STATUS_STATEMENT:
        return <StatusStatement employees={employees} reviews={reviews} penalties={penalties} />;
      case Page.SALARY_PROGRESSION:
        return <SalaryProgression employees={employees} setEmployees={setEmployees} />;
      ...
      case Page.LOANS:
        return <LoanManager employees={employees} setEmployees={setEmployees} loans={loans} setLoans={setLoans} />;
      case Page.BONUSES:
      ...
      case Page.SETTINGS:
        return currentUser?.role === 'admin' ? (
            <Settings departments={departments} setDepartments={setDepartments} getAllData={() => ({users, employees, leaves, loans, reviews, departments, attendance, penalties, bonuses, taxDebts, externalWorkers})} restoreData={(data) => { if(data.users) setUsers(data.users); if(data.employees) setEmployees(data.employees); if(data.leaves) setLeaves(data.leaves); if(data.loans) setLoans(data.loans); if(data.reviews) setReviews(data.reviews); if(data.departments) setDepartments(data.departments); if(data.attendance) setAttendance(data.attendance); if(data.penalties) setPenalties(data.penalties); if(data.bonuses) setBonuses(data.bonuses); if(data.taxDebts) setTaxDebts(data.taxDebts); if(data.externalWorkers) setExternalWorkers(data.externalWorkers); }} resetSystem={() => { setUsers(INITIAL_USERS); setEmployees(INITIAL_EMPLOYEES); setLeaves(INITIAL_LEAVES); setLoans(INITIAL_LOANS); setReviews(INITIAL_REVIEWS); setDepartments(INITIAL_DEPARTMENTS); setAttendance([]); setPenalties({}); setBonuses([]); setTaxDebts([]); setExternalWorkers([]); localStorage.clear(); handleLogout(); }} currentUser={currentUser!} updateUserPassword={updateUserPassword} setEmployees={setEmployees} setAttendance={setAttendance} setLeaves={setLeaves} setLoans={setLoans} setReviews={setReviews} setBonuses={setBonuses} setTaxDebts={setTaxDebts} setExternalWorkers={setExternalWorkers} setPenalties={setPenalties} />
        ) : <Dashboard employees={employees} attendance={attendance} leaves={leaves} loans={loans} setPage={setActivePage} reviews={reviews} bonuses={bonuses} penalties={penalties} taxDebts={taxDebts} />;
      default:
  ...
  return (
    <div className="flex flex-row min-h-screen bg-emerald-50 text-right font-sans relative">
      ...
      <main className="flex-1 overflow-auto h-screen relative">{renderContent()}</main>
